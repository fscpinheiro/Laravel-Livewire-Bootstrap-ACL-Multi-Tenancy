<div>
    ...
    <form id="UserForm" class="row g-3 {{ $form_show != 1 ? 'd-none' : '' }}" autocomplete="off"  wire:submit.prevent="saveUser">
        ...
        <div class="col-12">
          ...
          <select id="role_id_select" wire:key="role_id_select" wire:model.defer="role_id"  class="select2 form-select" title="Escolha um Papel para o usuário">
            <option value="">Escolha uma opção</option>
            @if($roles)
              @foreach ($roles as $role)
                <option value="{{ $role->id }}" data-color="{{ $role->cor }}">{{ $role->nome }} </option>
              @endforeach
            @endif
          </select>
          @error('role_id')
        ...
        <div class="col-12 col-md-4">
          ...
          <select id="situacao_id_select" wire:key="situacao_id_select" wire:model.defer="situacao" class="select2 form-select" data-allow-clear="true" title="Defina a situação inicial do Usuário">
            <option value="">Escolha uma opção</option>
          ...
          <div class="demo-inline-spacing mt-3">
              <div class="list-group">
                <div class="list-group-item list-group-item-action d-flex align-items-center cursor-pointer">
                  @if($perfil)
                  <img src="{{ asset('storage/perfil_uploads/' . $perfil) }}" alt="User Image" class="rounded me-3 w-px-100">
                  @else
                    ...
                    <div class="d-flex justify-content-between">
                      <div class="user-info">
                        ...
                        <div class="user-status">
                            <span class="badge badge-dot" style="background-color: {{ $color_st }}"></span>
                            <small>{{ $situacao_label }}</small>
                        </div>
                      </div>
                      <div class="add-btn">
